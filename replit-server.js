// ============================================
//  اسم فامیل آنلاین - سرور برای Replit.com
// ============================================
...
io.on('connection', (socket) => {
  ...
  socket.on('create_room', ({ playerName }) => {
    ...
    const room = {
      ...
      hostId: socket.id,
      players: {
        [socket.id]: { id: socket.id, name: playerName || 'بازیکن', score: 0 }
      },
      state: 'waiting',
      round: 1,
      maxRounds: 5,
  ...
  socket.on('submit_answers', ({ roomCode, answers }) => {
    ...
    io.to(roomCode).emit('answers_progress', {
      submitted: done,
      total: total,
    });
...
server.listen(PORT, () => {
  console.log(`🎮 Esm-Famil server running on port ${PORT}`);
});
